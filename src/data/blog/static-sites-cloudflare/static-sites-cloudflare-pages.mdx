...
2. Provision Base Infrastructure: In our example there is no base infrastructure. All you need is a cloudflare account and project.
  Cloudflare Account ID and API Token need to be setup as environment variables in GitHub.
3. Execute Build: We build and export as a static site. This step includes linting and type-checking.
4. Run integration tests: For demo purposes we have some (very) basic Jest integration tests.
5. Provision Staging Environment: Cloudflare pages comes with environments built-in, so this and step 8 are handled
by the following deployment step which sets up an environment.
6. Deploy to Staging Environment: We deploy the build to a staging environment using the official [GitHub Action](https://github.com/cloudflare/pages-action) for Cloudflare Pages.
...
cloudflare to target a staging environment. The step returns the url as an output.
...
9. Deploy to Production: Repeat step 6 but targeting the production environment. Specifying the main branch within Cloudflare ensures we
...
Applied to our scenario, the steps translate as follows: 
  ...
  2. If the PR includes source changes, we execute the build: Similarly to the main pipeline we use node to build and export our Next-JS site. This includes linting and
type checking.
  3. Run Integration Tests: We run our Jest tests.
  4. Provision Preview Environment: This functionality is again, built in to CloudFlare pages. When we execute the following steps
the [CloudFlare GitHub Action](https://github.com/cloudflare/pages-action) tool will handle provisioning an environment based on the PR name. 
  5. We deploy to a Preview Environment with a URL specific to this PR.
