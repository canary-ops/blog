...
This consists of a main branch and multiple short-lived feature branches which are reviewed merged using the Pull-Request mechanism.

## Workflows

This article uses [BPMN](https://www.bpmn.org/) as a diagram standard fo describing top-level Workflows.
Firstly we will look at the workflow triggered when a PR is merged into main or changes are directly pushed.
  ...
  deploy_to_preview:
    ...
    steps:
      ...
      - name: Get Deployment Token for Static Web App
        ...
        with:
          inlineScript: |
            APIKEY=$(az staticwebapp secrets list --name ${{ secrets.SWA_APP_NAME }} | jq -r '.properties.apiKey')
            echo "STATIC_DEPLOYMENT_TOKEN=$APIKEY" >> $GITHUB_ENV
  ...
  close_pull_request_job:
    ...
    steps:
      ...
      - name: Close Pull Request
        ...
        uses: Azure/static-web-apps-deploy@v1
        with:
          azure_static_web_apps_api_token: ${{ secrets.STATIC_DEPLOYMENT_TOKEN }}
          action: "close"
